...
- Observability: log strutturati (livelli info/warn/error) con request id; metriche dove serve.

## Error handling e logging
- Usa eccezioni di dominio per errori attesi (400), eccezioni generiche portano a 500.
- Logga solo ciò che serve a debuggare (no dati sensibili), includi contesto (asset_id, user, op).
- Mantieni un handler globale per formattare gli errori in modo consistente.
